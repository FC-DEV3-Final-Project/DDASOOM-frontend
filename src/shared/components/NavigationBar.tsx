...
const NavigationBar: React.FC = () => {
  ...
  const currentDropdown = hoveredLinkIndex !== null ? links[hoveredLinkIndex]?.dropdownItems : null
    ...
    >
      ...
      <div className="mx-auto flex w-full max-w-[1300px] min-w-[1000px] items-center justify-between px-5">
        ...
        <div className="flex items-center justify-start gap-14">
          {links.map((link, index) => (
            <div key={index} onMouseEnter={() => setHoveredLinkIndex(index)}>
              <NavigationLink
                text={link.text}
      ...
      {currentDropdown && (
        <div className="flex w-full justify-center bg-white shadow-md">
          <div className="grid h-[400px] w-[1300px] grid-cols-5 gap-8 px-8 py-6">
            {Object.entries(currentDropdown).map(([category, value], idx) => (
              <div key={idx}>
                {typeof value === 'string' ? (
                  <a
                    href={value}
                  ...
                  <>
                    <div className="text-red-40 mb-3 text-[17px] font-semibold">{category}</div>
