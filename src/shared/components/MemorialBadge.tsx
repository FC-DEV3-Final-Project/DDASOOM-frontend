...
const FlowerBlob = ({ className = '' }: { className?: string }) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 140 146" fill="none">
    <path
...
const MemorialBadge = ({ donor }: MemorialBadgeProps) => {
  return (
    <div className="relative h-[146px] w-[140px]">
      {/* 배경 SVG */}
      <FlowerBlob className="absolute inset-0 h-full w-full" />
      <div className="absolute top-[50%] left-[50%] h-[110px] w-[110px] translate-x-[-50%] translate-y-[-50%] rounded-full bg-white shadow-[0_0px_20px_0px_rgb(147,220,168)]"></div>
      {/* 오버레이 내용 */}
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <img src="/ribbon.png" alt="Memorial Ribbon" width={42} />
        <p className="text-[17px] font-semibold text-gray-800">
          {donor.name}
          <span className="ml-1 font-normal">님</span>
        </p>
        <p className="text-gray-60 mt-[-4px] text-[13px] font-bold">
