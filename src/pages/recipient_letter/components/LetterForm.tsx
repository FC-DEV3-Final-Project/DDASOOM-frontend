...
const ORGAN_OPTIONS = ['신장', '간', '심장', '폐', '췌장', '소장', '손 팔', '안구', '인체조직']
...
const YEARS = Array.from({ length: 50 }, (_, i) => currentYear - i) // 최근 50년치
const LetterForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!writer.trim()) {
      errors.push('수혜자 이름을 입력해주세요.')
    } else if (!/^[가-힣a-zA-Z]{1,10}$/.test(writer)) {
      errors.push('수혜자 이름은 한글 또는 영문 10자 이내로 입력해주세요.')
    }
